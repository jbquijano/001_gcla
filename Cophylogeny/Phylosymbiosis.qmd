...
# Filtered host
hosts_with_its2 <- unique(metadata_1$corrected_id)

host_tree_pruned <- drop.tip(
  host_tree,
  setdiff(host_tree$tip.label, hosts_with_its2)
)

# Which are missing? # TG50 and TG65; no COI sequence but we're sure that they're TG
setdiff(hosts_with_its2, host_tree_pruned$tip.label)

# Filter Symbiodiniaceae data
metadata_2 <- 
  metadata_1 |> 
  filter(corrected_id %in% host_tree$tip.label)

its2_2 <- 
  metadata_2 |>
  left_join(its2_1) |> 
  select(!sample_name) |> 
  rename(sample_name = corrected_id) |> 
  select(!c(2:5))  |>
  arrange(sample_name) |> 
...
```{r}
its2_dist <- 
  vegdist(its2_2, method = "bray")

its2_hc <- 
  hclust(its2_dist, method = "average")

its2_tree <- as.phylo(its2_hc)

rf_res <- RFmeasures(host_tree_pruned, its2_hc)
```
...
```{r}
host_dist <- 
  cophenetic(host_tree_pruned)

shared_ids <- intersect(rownames(host_dist), rownames(its2_dist))

host_dist <- host_dist[shared_ids, shared_ids]
its2_dist <- its2_dist[shared_ids, shared_ids]

identical(rownames(host_dist), rownames(its2_dist))
